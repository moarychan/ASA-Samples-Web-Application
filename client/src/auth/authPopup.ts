...
function selectAccount() {
    ...
    if (currentAccounts.length < 1) {
        ...
        const accounts = currentAccounts.filter(account => {
                if (account.homeAccountId.toUpperCase().includes(b2cPolicies.names.signUpSignIn.toUpperCase())) {
                    const claims = account.idTokenClaims;
                    // @ts-ignore
                    if (claims && claims.iss.toUpperCase().includes(b2cPolicies.authorityDomain.toUpperCase())
                        && claims.aud === msalConfig.auth.clientId) {
                        return true;
                    }
                }
                return false;
